...
class MovingAverageCrossover(Signal):
    ...
    def get_action(self, df):
        ...
        if not 'sma50_gt_sma200_cross_over' in df.columns:
            df['sma50_gt_sma200_cross_over'] = df.sma50_gt_sma200.ne(df.sma50_gt_sma200.shift())
        df.loc[df['sma50_gt_sma200'] == False, 'sma50_gt_sma200_cross_over'] = False

        if not 'sma50_lt_sma200' in df.columns:
            df['sma50_lt_sma200'] = df.sma50 < df.sma200

        if not 'sma50_lt_sma200_cross_over' in df.columns:
            df['sma50_lt_sma200_cross_over'] = df.sma50_lt_sma200.ne(df.sma50_lt_sma200.shift())

        df.loc[df['sma50_lt_sma200'] == False, 'sma50_lt_sma200_cross_over'] = False
        ...
        if latest_interval['sma50_gt_sma200_cross_over'].values[0] == True:
            self.log.debug(f'sma50 {latest_interval["sma50"].values[0]} has crossed over sma200 {latest_interval["sma200"].values[0]}')
            action = SignalAction.BUY
        elif latest_interval['sma50_lt_sma200_cross_over'].values[0] == True:

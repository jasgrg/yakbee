...
class ExponentialMovingAverageCrossover(Signal):
    ...
    def get_action(self, df):
        ...
        if not 'sma8_gt_ema12_cross_over' in df.columns:
            df['sma8_gt_ema12_cross_over'] = df.ema8_gt_ema12.ne(df.ema8_gt_ema12.shift())
            df.loc[df['ema8_gt_ema12'] == False, 'sma8_gt_ema12_cross_over'] = False

        if not 'ema8_lt_ema12' in df.columns:
            df['ema8_lt_ema12'] = df.ema8 < df.ema12
        if not 'ema8_lt_ema12_cross_over' in df.columns:
            df['ema8_lt_ema12_cross_over'] = df.ema8_lt_ema12.ne(df.ema8_lt_ema12.shift())
            df.loc[df['ema8_lt_ema12'] == False, 'ema8_lt_ema12_cross_over'] = False

        if not 'ema12_gt_ema26' in df.columns:
            df['ema12_gt_ema26'] = df.ema12 > df.ema26
        if not 'ema12_gt_ema26_cross_over' in df.columns:
            df['ema12_gt_ema26_cross_over'] = df.ema12_gt_ema26.ne(df.ema12_gt_ema26.shift())
            df.loc[df['ema12_gt_ema26'] == False, 'ema12_gt_ema26_cross_over'] = False

        if not 'ema12_lt_ema26' in df.columns:
            df['ema12_lt_ema26'] = df.ema12 < df.ema26
        if not 'ema12_lt_ema26_cross_over' in df.columns:
        ...
        if latest_interval['ema12_gt_ema26_cross_over'].values[0] == True:
            self.log.debug(f'ema12 {latest_interval.ema12.values[0]} has crossed over ema26 {latest_interval.ema26.values[0]}')
            action = SignalAction.BUY
        elif latest_interval['ema12_lt_ema26_cross_over'].values[0] == True:
    ...
    def render(self, df):
        filename = 'graphs/exponential_moving_average_crossover.png'
